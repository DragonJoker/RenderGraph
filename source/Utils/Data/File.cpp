...
namespace utils
{
		...
		, m_fileFullPath{ p_fileName }
	{
		...
		{
			m_length = 0;
			utils::fileSeek( m_file, 0, SEEK_END );
			m_length = utils::fileTell( m_file );
			utils::fileSeek( m_file, 0, SEEK_SET );
		}
		else
		{
	...
	int File::seek( long long p_offset, OffsetMode p_origin )
	{
		...
		if ( m_file )
		{
			...
			case OffsetMode::eBeginning:
				iReturn = utils::fileSeek( m_file, p_offset, SEEK_SET );
				m_cursor = p_offset;
				break;

			case OffsetMode::eCurrent:
				iReturn = utils::fileSeek( m_file, p_offset, SEEK_CUR );
				m_cursor += p_offset;
				break;

			case OffsetMode::eEnd:
				iReturn = utils::fileSeek( m_file, p_offset, SEEK_END );
	...
	long long File::getLength()
	{
		CHECK_INVARIANTS();
		m_length = 0;
		long long llPosition = utils::fileTell( m_file );
		utils::fileSeek( m_file, 0, SEEK_END );
		m_length = utils::fileTell( m_file );
		utils::fileSeek( m_file, llPosition, SEEK_SET );
		CHECK_INVARIANTS();
		return m_length;
	}
	...
	long long File::tell()
	{
		...
		if ( m_file )
		{
			llReturn = utils::fileTell( m_file );
		}
