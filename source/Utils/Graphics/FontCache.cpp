#include "Utils/Graphics/FontCache.hpp"

#include "Utils/Graphics/Font.hpp"
...
namespace utils
{
	namespace
	{
		...
		inline void doReportCreation( utils::String const & name )
		{
			utils::Logger::logTrace( utils::makeStringStream()
				<< INFO_CACHE_CREATED_OBJECT
				<< cuT( "Font: " )
				<< name );
		}

		inline void doReportDuplicate( utils::String const & name )
		{
			utils::Logger::logWarning( utils::makeStringStream()
	...
	FontSPtr FontCache::add( utils::String const & p_name, FontSPtr p_font )
	{
		auto lock = makeUniqueLock( *this );
