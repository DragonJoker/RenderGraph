...
#define CHECK_FT_ERR( func, ... ) CheckErr( func( __VA_ARGS__ ), #func )
			...
			: public Font::SFontImpl
		{
			...
			Glyph loadGlyph( char32_t p_c32 )override
			{
				...
				if ( advance < size.width )
				{
					advance = size.width + bearing.x;
				}
