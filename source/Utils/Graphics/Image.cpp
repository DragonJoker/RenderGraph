...
namespace utils
{
	Image::Image( String const & p_name, Size const & p_size, ashes::Format p_format, ByteArray const & p_buffer, ashes::Format p_eBufferFormat )
	...
	Image::Image( String const & p_name, Size const & p_size, ashes::Format p_format, uint8_t const * p_buffer, ashes::Format p_eBufferFormat )
		: Resource< Image >( p_name )
	...
	Image & Image::setPixel( uint32_t x, uint32_t y, uint8_t const * p_pPixel, ashes::Format p_format )
	{
		CHECK_INVARIANTS();
	...
	Image & Image::setPixel( uint32_t x, uint32_t y, RgbColour const & p_clrColour )
	{
		...
		uint8_t * pDst = &( *m_buffer->getAt( x, y ) );
		PF::convertPixel( ashes::Format::eR8G8B8A8_UNORM, pSrc, getPixelFormat(), pDst );
		CHECK_INVARIANTS();
		return * this;
	}
	...
	Image & Image::setPixel( uint32_t x, uint32_t y, RgbaColour const & p_clrColour )
	{
		...
		uint8_t const * pSrc = ptComponents.constPtr();
		uint8_t * pDst = &( *m_buffer->getAt( x, y ) );
		PF::convertPixel( ashes::Format::eR8G8B8_UNORM, pSrc, getPixelFormat(), pDst );
		CHECK_INVARIANTS();
		return * this;
	}

	void Image::getPixel( uint32_t x, uint32_t y, uint8_t * p_pPixel, ashes::Format p_format )const
	{
		CHECK_INVARIANTS();
		REQUIRE( x < getWidth() && y < getHeight() && p_pPixel );
	...
	RgbaColour Image::getPixel( uint32_t x, uint32_t y )const
	{
		...
		PF::convertPixel( getPixelFormat(), pSrc, ashes::Format::eR8G8B8A8_UNORM, pDst );
		CHECK_INVARIANTS();
	...
	{
		CHECK_INVARIANTS();
		REQUIRE( getIntersection( { { 0, 0 }, { getWidth(), getHeight() } }, rcRect ) == Intersection::eIn );
		Size ptSize{ rcRect.size };
		// Création de la sous-image à remplir
		Image img( m_name + cuT( "_Sub" ) + string::toString( rcRect.position.x ) + cuT( "x" ) + string::toString( rcRect.position.y ) + cuT( ":" ) + string::toString( ptSize.width ) + cuT( "x" ) + string::toString( ptSize.height ), ptSize, getPixelFormat() );
		// Calcul de variables temporaires
		uint8_t const * pSrc = &( *m_buffer->getAt( rcRect.position.x, rcRect.position.y ) );
		...
		for ( auto i = rcRect.position.x; i < int32_t( rcRect.position.x + rcRect.size.width ); ++i )
		{
