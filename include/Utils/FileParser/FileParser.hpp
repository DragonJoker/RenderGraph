...
*/
#ifndef ___Utils_FILE_PARSER_H___
#define ___Utils_FILE_PARSER_H___
...
namespace utils
{
	 ...
	 */
	typedef std::function< bool( utils::FileParser * p_parser, ParserParameterArray const & p_params ) > ParserFunction;

#define DO_WRITE_PARSER_NAME( funcname )\
	funcname( utils::FileParser * p_parser, utils::ParserParameterArray const & p_params )
...
#define IMPLEMENT_ATTRIBUTE_PARSER( funcname )\
	...
	{\
		bool result = false;\
		utils::FileParserContextSPtr p_context = p_parser->getContext();
...
#define MAKE_SECTION_NAME( a, b, c, d )\
	...
	*/
	...
	public:
		 ...
		 */
		FileParser( uint32_t rootSectionId );
		/**
		 *\~english
		 *\brief		Destructor.
		 *\~french
		 ...
		 */
		bool parseFile( Path const & path );
		/**
		 *\~english
		 *\brief		Parsing function.
		 *\param[in]	path	The file access path.
		 ...
		 *\return		\p true si tout s'est bien passé.
		 */
		bool parseFile( Path const & path, String const & content );
		/**
		 *\~english
		 ...
		 */
		void parseError( String const & error );
		/**
		 *\~english
		 ...
		 */
		void parseWarning( String const & warning );
		/**
		 *\~english
		 ...
		 */
		bool checkParams( String const & params
		 ...
		 */
		void addParser( uint32_t section
			, String const & name
	...
	protected:
		 ...
		 *\param[in]	path	Le chemin d'accès au fichier.
		 */
		virtual void doInitialiseParser( utils::Path const & path ) = 0;
		/**
		 *\~english
		 ...
		 */
		virtual bool doDelegateParser( String const & line ) = 0;
		/**
		 *\~english
		 ...
		 */
		virtual bool doDiscardParser( String const & p_error ) = 0;
		/**
		 *\~english
		 ...
		 *\return		Le nom.
		 */
		virtual utils::String doGetSectionName( uint32_t p_section ) = 0;

	private:
		bool doParseScriptLine( String & p_line );
		bool doParseScriptBlockEnd();
		bool doInvokeParser( utils::String & p_line, AttributeParserMap const & p_parsers );
		void doEnterBlock();
		void doLeaveBlock();
