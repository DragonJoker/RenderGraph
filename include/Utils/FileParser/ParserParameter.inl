...
#include "Utils/Design/ArrayView.hpp"
...
namespace utils
{
	...
	inline bool parseValues( String & params, size_t count, T * value )
	{
		...
		try
		{
			...
			{
				Logger::logWarning( utils::makeStringStream() << cuT( "Couldn't parse from " ) << params );
			}
		}
		catch ( std::exception & p_exc )
		{
			Logger::logError( utils::makeStringStream() << cuT( "Couldn't parse from " ) << params << cuT( ": " ) << string::stringCast< xchar >( p_exc.what() ) );
		}
		...
		if ( result )
		{
			for ( auto & value : makeArrayView( values, values + count ) )
			{
				if ( value < range.getMin()
					|| value > range.getMax() )
				{
					Logger::logWarning( utils::makeStringStream() << cuT( "Value " ) << value << cuT( " is outside of range [" ) << range.getMin() << cuT( ", " ) << range.getMax() << cuT( "]" ) );
	...
	struct ValueParser< Type, typename std::enable_if< Type == ParameterType::ePixelFormat >::type >
	{
		...
		static inline bool parse( String & params, ValueType & value )
		{
			...
			if ( values.size() )
			{
				...
				result = value != ashes::Format::eRange;
	...
	*/
	...
	struct ValueParser< Type, typename std::enable_if< Type == ParameterType::eSize >::type >
	{
		...
		static inline bool parse( String & params, ValueType & value )
		{
			...
			{
				result = utils::System::getScreenSize( 0, value );
			}
			else
			{
