...
*/
#ifndef ___Utils_PIXEL_H___
#define ___Utils_PIXEL_H___
...
namespace utils
{
	...
	\remark		Takes a ashes::Format as a template argument to determine size and format.
	\~french
	\brief		Définition d'un pixel.
	\remark		L'argument template ashes::Format sert a déterminer la taille et le format du pixel.
	*/
	 ...
	 */
	template < ashes::Format FT, ashes::Format FU >
	...
	template < ashes::Format FT, ashes::Format FU >
	Pixel< FT > operator*( Pixel< FT > const & p_lhs, Pixel< FU > const & p_rhs );
	template < ashes::Format FT, typename U >
	...
	*/
	typedef Pixel< ashes::Format::eR8G8B8A8_UNORM > UbPixel;
}
