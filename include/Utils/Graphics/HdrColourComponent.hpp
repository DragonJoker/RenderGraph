...
*/
#ifndef ___Utils_HDR_COLOUR_COMPONENT_H___
#define ___Utils_HDR_COLOUR_COMPONENT_H___
...
namespace utils
{
	...
	\date		07/10=1/2016
	\~english
	\brief		Defines a HDR colour PixelComponents (R, G, B or A) to be used in utils::HdrRgbColour or utils::HdrRgbaColour.
	\remark		Holds conversion operators to be converted either into float or uint8_t, with corresponding operations
	\~french
	\brief		Représente une composante de couleur HDR (R, V, B ou A) pour l'utilisation dans utils::HdrRgbColour ou utils::HdrRgbaColour.
	...
	*/
	 ...
	 */
	bool operator==( HdrColourComponent const & p_lhs, HdrColourComponent const & p_rhs );
	/**
	 *\~english
	 ...
	 *\return		Resultat de p_lhs + p_rhs
	 */
	float operator+( HdrColourComponent const & p_lhs, HdrColourComponent const & p_rhs );
	/**
	 *\~english
