...
*/
#ifndef ___Utils_ASSERTION_H___
#define ___Utils_ASSERTION_H___
...
#		define CASTOR_ASSERT( pred, text )
	if ( !( pred ) )\
	{\
		throw utils::AssertException( ( text ), __FILE__, __FUNCTION__, __LINE__ );\
	}
...
namespace utils
{
	...
	inline void cuAssert( Expr const & expr, char const * const description )
	{
		if ( !expr )
		{
			...
			Logger::logError( std::stringstream() << ashes::Debug::Backtrace{} );
			assert( false );
		}
	}
}

#		define CASTOR_ASSERT( pred, text ) utils::cuAssert( pred, text )
