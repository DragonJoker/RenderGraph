...
*/
#ifndef ___Utils_FILE_H___
#define ___Utils_FILE_H___

#include <fstream>
#include "Path.hpp"
#include "Utils/Utils.hpp"
#include "Utils/Design/FlagCombination.hpp"
...
namespace utils
{
	 ...
	 */
	bool fileOpen( FILE *& p_pFile, char const * p_pszPath, char const * p_pszMode );
	/**
	 *\~english
	 *\brief		Opens a file
	 *\param[out]	p_pFile		Receives the file descriptor
	 ...
	 *\return		\p true en cas de réussite
	 */
	bool fileOpen64( FILE *& p_pFile, char const * p_pszPath, char const * p_pszMode );
	/**
	 *\~english
	 ...
	 *\return		\p true en cas de réussite
	 */
	bool fileSeek( FILE * p_pFile, int64_t p_offset, int p_iOrigin );
	/**
	 *\~english
	...
	*/
	...
	public:
		...
		*/
			...
			: uint8_t
		{
			...
			eEnd,
			Utils_ScopedEnumBounds( eBeginning )
		};
		/*!
		\~english
	...
	public:
		 ...
		 */
		static bool listDirectoryFiles( Path const & p_folderPath, PathArray & p_files, bool p_recursive = false );
		/**
		 *\~english
		 ...
		 */
		static bool directoryCreate( Path const & p_path, FlagCombination< CreateMode > const & p_flags = CreateMode::eAllRWX );
		/**
		 *\~english
	...
	protected:
		...
		uint64_t doWrite( uint8_t const * p_buffer, uint64_t p_uiSize );
		uint64_t doRead( uint8_t * p_buffer, uint64_t p_uiSize );
