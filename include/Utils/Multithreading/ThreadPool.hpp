...
*/
#ifndef ___Utils_ThreadPool_H___
#define ___Utils_ThreadPool_H___
...
namespace utils
{
	...
	*/
	...
	public:
		 ...
		 */
		explicit ThreadPool( size_t p_count );
		/**
		 *\~english
		 *\brief		Destructor.
		 *\~french
		 ...
		 */
		bool waitAll( utils::Milliseconds const & p_timeout )const;
		/**
		 *\~english
		 ...
		 */
		void pushJob( WorkerThread::Job p_job );
		/**
		 *\~english
	...
	private:
		...
		std::vector< ashes::SignalConnection< WorkerThread::OnEnded > > m_endConnections;
	};
}
