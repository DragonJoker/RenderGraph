...
namespace utils
{
	...
	Point< T, TCount >::TextLoader::TextLoader()
		: utils::TextLoader< Point< T, TCount > >()
	{
	}
	...
	Point< T, TCount >::TextWriter::TextWriter( String const & tabs )
		: utils::TextWriter< Point< T, TCount > >( tabs )
	{
	}
	...
	bool Point< T, TCount >::TextWriter::operator()( Point< T, TCount > const & object, TextFile & file )
	{
		...
		utils::TextWriter< Point< T, TCount > >::checkError( result, "Point value" );
		return result;
	}
...
template< typename T, uint32_t TCount >
inline utils::String & operator<<( utils::String & out, utils::Point< T, TCount > const & in )
{
...
template< typename T, uint32_t TCount >
inline utils::String & operator>>( utils::String & in, utils::Point< T, TCount > & out )
{
...
inline std::basic_ostream< CharType > & operator<<( std::basic_ostream< CharType > & out, utils::Point< T, TCount > const & in )
{
	if ( TCount )
	{
		out << in[0];
...
template< typename T, uint32_t TCount, typename CharType >
inline std::basic_istream< CharType > & operator>>( std::basic_istream< CharType > & in, utils::Point< T, TCount > & out )
{
	for ( uint32_t i = 0; i < TCount; i++ )
	{
		in >> out[i];
