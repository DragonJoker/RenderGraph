...
*/
#ifndef ___Utils_SQUARE_MATRIX_H___
#define ___Utils_SQUARE_MATRIX_H___
...
namespace utils
{
	...
	**/
	/**@{*/
	template< typename CharT, typename T, uint32_t Count > std::basic_ostream< CharT > & operator<<( std::basic_ostream< CharT > & p_streamOut, utils::SquareMatrix< T, Count > const & p_matrix );
	template< typename CharT, typename T, uint32_t Count > std::basic_istream< CharT > & operator>>( std::basic_istream< CharT > & p_streamIn, utils::SquareMatrix< T, Count > & p_matrix );
	/**@}*/
}
