...
namespace utils
{
	...
	Coords< T, Count >::TextLoader::TextLoader()
		: utils::TextLoader< Coords< T, Count > >()
	{
	}
	...
	Coords< T, Count >::TextWriter::TextWriter( String const & tabs )
		: utils::TextWriter< Coords< T, Count > >( tabs )
	{
	}
	...
	{
		StringStream streamWord{ utils::makeStringStream() };
		...
		utils::TextWriter< Coords< T, Count > >::checkError( result, "Coords value" );
		return result;
	}
...
template< typename T, uint32_t Count >
inline utils::String & operator<<( utils::String & out, utils::Coords< T, Count > const & in )
{
	utils::StringStream stream{ utils::makeStringStream() };
	stream << in;
	out += stream.str();
	return out;
}
...
template< typename T, uint32_t Count >
inline utils::String & operator>>( utils::String & in, utils::Coords< T, Count > & out )
{
	utils::StringStream stream( in );
	stream >> out;
	in = stream.str();
	return in;
}
...
inline std::basic_ostream< CharType > & operator<<( std::basic_ostream< CharType > & out, utils::Coords< T, Count > const & in )
{
	if ( Count )
	{
		out << in[0];
...
template< typename T, uint32_t Count, typename CharType >
inline std::basic_istream< CharType > & operator>>( std::basic_istream< CharType > & in, utils::Coords< T, Count > & out )
{
	for ( uint32_t i = 0; i < Count; i++ )
	{
		in >> out[i];
