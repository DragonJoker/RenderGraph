...
namespace utils
{
	namespace matrix
	{
		...
		template< typename T, typename U >
		Point3< U > getTransformed( utils::Matrix4x4< T > const & matrix
			, utils::Point3< U > const & value )
		{
			Point3< U > result;
		...
		template< typename T, typename U >
		static Quaternion getTransformed( utils::Matrix4x4< T > const & matrix
			, utils::QuaternionT< U > const & value )
		{
			return matrix * value;
		}
			...
			, Point3< U > const & up )
		{
			...
			Point3< T > s( point::getNormalised( utils::point::cross( f, up ) ) );
			Point3< T > u( utils::point::cross( s, f ) );
			matrix.setIdentity();
			matrix[0][0] = s[0];
			matrix[0][1] = u[0];
			matrix[0][2] = -f[0];
