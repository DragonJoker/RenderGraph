...
*/
#ifndef ___Utils_Macros___
#define ___Utils_Macros___

#include "Utils/config.hpp"
#include "Utils/Config/PlatformConfig.hpp"
#include "Utils/Config/CompilerConfig.hpp"
...
#define Utils_Coucou utils::Logger::logDebug( cuT( "Coucou % 4d : %s @ line %d" ), __COUNTER__, __FUNCTION__, __LINE__ );
...
#	endif
#elif defined( Utils_CompilerCLANG )
...
#		define cvsnprintf( buf, sz, cnt, fmt, arg ) vsnprintf( buf, cnt, fmt, arg )
#	endif
#elif defined( Utils_CompilerGNUC )
#	if !defined( Utils_PlatformWin32 )
#		define _FILE_OFFSET_BITS 64
#		define cvsnprintf( buf, sz, cnt, fmt, arg ) vsnprintf( buf, cnt, fmt, arg )
#	else
...
#		define cvsnprintf( buf, sz, cnt, fmt, arg ) vsnprintf( buf, cnt, fmt, arg )
#	endif
#elif defined( Utils_CompilerBORLAND )
#	if !defined( Utils_PlatformWin32 )
#		define _FILE_OFFSET_BITS 64
#		define cvsnprintf( buf, sz, cnt, fmt, arg ) vsnprintf( buf, cnt, fmt, arg )
#	else
...
#define DECLARE_COLLECTION( elem, key, name )\
	using name##Collection = utils::Collection< elem, key >;\
...
#define Utils_EnumBounds( EnumName, EnumMin )\
	EnumName##_COUNT,\
	EnumName##_MIN = EnumMin,\
	EnumName##_MAX = EnumName##_COUNT - 1

#define Utils_ScopedEnumBounds( EnumMin )\
	eCount,\
	eMin = EnumMin,\
	eMax = eCount - 1
...
#define IMPLEMENT_FLAGS( FlagType )\
	...
	inline constexpr typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type operator|( FlagType p_lhs, FlagType p_rhs )\
	{\
		return typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type( p_lhs ) | typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type( p_rhs );\
	}\
	inline constexpr typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type operator|( typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type p_lhs, FlagType p_rhs )\
	{\
		return p_lhs | typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type( p_rhs );\
	}\
	inline constexpr typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type operator|( FlagType p_lhs, typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type p_rhs )\
	{\
		return typename utils::BaseTypeFromSize< sizeof( FlagType ) >::Type( p_lhs ) | p_rhs;\
	}

#define IMPLEMENT_CLASS_FLAGS( Class, FlagType )\
	using Class##FlagType##s = utils::FlagCombination< Class::FlagType >;\
	inline constexpr typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type operator|( Class::FlagType p_lhs, Class::FlagType p_rhs )\
	{\
		return typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type( p_lhs ) | typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type( p_rhs );\
	}\
	inline constexpr typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type operator|( typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type p_lhs, Class::FlagType p_rhs )\
	{\
		return p_lhs | typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type( p_rhs );\
	}\
	inline constexpr typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type operator|( Class::FlagType p_lhs, typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type p_rhs )\
	{\
		return typename utils::BaseTypeFromSize< sizeof( Class::FlagType ) >::Type( p_lhs ) | p_rhs;\
	}
